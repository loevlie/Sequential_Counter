#!/usr/bin/env python3
"""
Memory-efficient training with GradCAM tracking and GIF generation.
Keeps vision encoder frozen and visualizes attention evolution.
"""
...
class FSC147Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            'image_id': img_id,
            'points': points  # Include points for visualization
        }


class GradCAMTrainer:
    """Trainer with GradCAM visualization capabilities"""
    ...
    def __init__(self, model_name="Qwen/Qwen3-VL-2B-Instruct", device=None, use_bfloat16=False):
        ...
        # Freeze vision encoder (never unfreeze)
    ...
    def compute_gradcam(self, image: Image.Image, target_size: int = 224):
        """
        Compute GradCAM-style attention map.
        This works even with frozen vision encoder!
        """
        self.model.eval()
        ...
        if "pixel_values" in inputs:
            pixel_values = inputs["pixel_values"].to(self.model_dtype).requires_grad_(True)
        ...
        with torch.enable_grad():
            ...
            if hasattr(outputs, 'logits'):
                ...
                # Get gradients
                grads = pixel_values.grad.data

                # Process gradients
                if grads.dim() == 4:  # [batch, channels, height, width]
                    attention_map = grads[0].abs().mean(dim=0).cpu().numpy()
                elif grads.dim() == 3:  # [batch, num_patches, hidden_dim]
                    batch_size, num_patches, hidden_dim = grads.shape
                    patch_size = int(np.sqrt(num_patches))
                    if patch_size * patch_size == num_patches:
                        grads_reshaped = grads[0].reshape(patch_size, patch_size, hidden_dim)
                        attention_map = grads_reshaped.abs().mean(dim=-1).cpu().numpy()
                    else:
                        attention_map = grads[0].abs().mean(dim=-1).cpu().numpy()
                ...
                if attention_map.shape[0] != target_size:
...
def train_epoch_with_gradcam(trainer: GradCAMTrainer,
        ...
        try:
            ...
            if (batch_idx + 1) % gradient_accumulation_steps == 0:
                ...
                if not has_nan_grad:
                    ...
                    total_count_loss += accumulated_loss
                    num_valid_batches += 1
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader, desc="Validation")):
            try:
                ...
                generated_ids = trainer.model.generate(**inputs, max_new_tokens=10, do_sample=False, temperature=0.1)
                generated_text = trainer.processor.decode(generated_ids[0], skip_special_tokens=True)
                predicted_count = extract_count_from_text(generated_text)
...
def main():
    parser = argparse.ArgumentParser(description='Training with GradCAM tracking and GIF generation')
    ...
    parser.add_argument('--num_track_samples', type=int, default=5, help='Number of samples to track')
    parser.add_argument('--gif_duration', type=float, default=0.5, help='Duration per frame in GIF (seconds)')

    args = parser.parse_args()

    print("=" * 50)
    print("Training with GradCAM Tracking")
    print("=" * 50)
    print(f"Vision encoder: FROZEN (will never unfreeze)")
    print(f"Tracking {args.num_track_samples} samples for GradCAM evolution")
    ...
    if args.use_wandb and WANDB_AVAILABLE:
        wandb.init(
            ...
            name=f"gradcam_lr{args.lr}_ga{args.gradient_accumulation}",
            config=vars(args)
        )
    ...
    # Select samples to track (from validation set for consistency)
    ...
    # Create GIFs for each tracked sample
    print("\n" + "=" * 50)
    print("Creating GIFs from tracked samples...")
    print("=" * 50)

    for sample_id in track_samples:
        sample_dir = os.path.join(args.vis_dir, sample_id.replace('.jpg', ''))
        if os.path.exists(sample_dir):
            # Get all epoch images sorted
    ...
    print("\n" + "=" * 50)
    print("GradCAM Evolution Complete!")
    print(f"Visualizations saved to: {args.vis_dir}")
    print(f"Look for *_evolution.gif files to see attention changes over training")
    print("=" * 50)
    ...
    if args.use_wandb and WANDB_AVAILABLE:
        ...
        for sample_id in track_samples:
            ...
            if os.path.exists(gif_path):
                wandb.log({f"gradcam_evolution/{sample_id}": wandb.Video(gif_path, fps=1/args.gif_duration, format="gif")})
